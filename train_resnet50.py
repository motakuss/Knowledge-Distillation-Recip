...
from src.model import TeacherModel, StudentModel, resnet50
    ...
    for i in range(20):
        ...
        for epoch in range(epochs):
            ...
            if score <= val_acc:
                print('test')
                score = val_acc
                torch.save(teacher.state_dict(), './logs/train_resnet50/resnet50_param' + str(i) + '.pth') 
        ...
        with open('./logs/train_resnet50/hist'+str(i)+'.pickle', mode='wb') as f:
            pickle.dump(teacher_hist, f)
            
        # teacher test
        teacher.load_state_dict(torch.load('./logs/train_resnet50/resnet50_param' + str(i) + '.pth'))
        test = {'acc': [], 'loss': []}
        teacher.eval()
        test_loss = 0.
        test_acc = 0.
        ...
        with open('./logs/train_resnet50/test'+str(i)+'.pickle', mode='wb') as f:
            pickle.dump(test, f)
