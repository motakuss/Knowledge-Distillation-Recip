    ...
    for i in range(20):
        ...
        for epoch in range(epochs):
            ...
            if score <= val_acc:
                print('test')
                score = val_acc
                torch.save(base_student.state_dict(), './logs/train_base/base_param' + str(i) + '.pth') 
        ...
        with open('./logs/train_base/hist'+str(i)+'.pickle', mode='wb') as f:
            pickle.dump(base_hist, f)

        base_student.load_state_dict(torch.load('./logs/train_base/base_param' + str(i) + '.pth'))
        base_student.eval()
        test = {'acc': [], 'loss': []}
        test_loss = 0.
        test_acc = 0.
        ...
        with open('./logs/train_base/test'+str(i)+'.pickle', mode='wb') as f:
            pickle.dump(test, f)
